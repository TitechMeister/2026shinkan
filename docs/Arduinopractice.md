...
※このコードでは、loop が超高速で回っているため、ボタンを押していなくても常に「ボタンが◯回押されました」という文字がシリアルモニタに延々と出力され続けます。

### 05-5. 演習問題

今の example_code_5.ino のコードでは、ボタンを押していない時も含めて、常にシリアルモニタへメッセージが送信され続けてしまい読みにくくなっています。

「ボタンが押されてカウントが増えた瞬間（指が離れてwhileを抜けた直後など）」だけ、つまりカウントが変化した時のみ 1回だけシリアルモニタに「ボタンが◯回押されました」と送信されるようにプログラムを修正してください。
